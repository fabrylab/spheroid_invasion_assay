...
def plotting_invasion_profiles1(res, meta_info, outputfolder_path, filename):
    ...
    factor_x=0.1 # factor for croping in x direction
    factor_y=0.1 # factor for cropping in y direction
    ...
    ax1 = fig.add_axes([0.075, 0.2, 0.25, 0.6])
  ...
  #  ax1.subplot(grid[14:86, 0:2])
    ...
    ax1.semilogy(x, dens[int(np.ceil(rad/px_um)):max_ind], linewidth=2, color='C0')  # show all data
    ax1.semilogy(x_fit, p[2] / (1 + np.exp((x_fit - p[0]) / p[1])), linewidth=2, color='C1')  # show fit, but only for x>rad
    ax1.fill_between([0, rad], [0, 0], [1, 1], facecolor=(0.729, 0.729, 0.729), edgecolor="none", linewidth=0) # fills area in plot represetning the blob
    ax1.fill_between([rad, p[0]], [0, 0], [1, 1], facecolor=("#C3E994"), edgecolor="none", linewidth=0) # fills area in plot  representing distance to lambda point

    ax1.set_ylim([1e-4, 1])
    ax1.set_xticks([0, 200, 400, 600, 800, 1000])
    ax1.set_yticks([1, 1e-1, 1e-2, 1e-3, 1e-4])
    ax1.text(610, 0.1, '$\lambda$=%.0f $\mu$m' % p[1], fontsize = 16)

    ax1.text(610, 0.3, 'd/2=%.0f $\mu$m' % inv_front, fontsize = 16)
    ax1.set_xlabel('distance from center ($\mu$m)')
  ...
  #  overlay_mask[mask] = 1.  # fill in everything except spheroid region (invert by deleting the "~"-sign)
    ...
    ax2 = fig.add_axes([0.375, 0.2, 0.25, 0.6])
    ax2.imshow(crop_image(img,factor_x,factor_y), cmap='gray')  # display image in greyscale
    ax2.imshow(crop_image(overlay_mask,factor_x,factor_y), cmap=custom_cmap1, alpha=0.5)  # display overlay, partially transparent
    ax2.axis('off')

    ax3 = fig.add_axes([0.675, 0.2, 0.25, 0.6])
    ax3.imshow(crop_image(img1,factor_x,factor_y), cmap='gray')  # display image in greyscale
    ax3.imshow(crop_image(blob_edge_overlay,factor_x,factor_y), cmap=custom_cmap2, alpha=0.5) # display the edge of the detected blob
    ax3.imshow(crop_image(d_half_ring,factor_x,factor_y), cmap=custom_cmap3, alpha=0.8)
...
#    ax3.arrow([img1.shape[1]-(img1.shape[1]*2*factor_x) - 250, img1.shape[1]-(img1.shape[1]*2*factor_x) - 250 + 500 / px_um], [img_norm.shape[0] - 30,img_norm.shape[0] - 30], lw=4,
 #            c='white')
    ax3.arrow(img1.shape[1]-(img1.shape[1]*2*factor_x) - 210, img1.shape[1]-(img1.shape[1]*2*factor_x)  - 340, 500 / px_um,0,color="white", length_includes_head=False, head_width=0., width=10)
    ax3.text(img1.shape[1]-(img1.shape[1]*2*factor_x)  - 210, img1.shape[0]-(img1.shape[0]*2*factor_y) - 20, '500 $\mu$m', color='white', fontsize=12)
    ax3.text(img1.shape[1]-(img1.shape[1]*2*factor_x) - 431, img1.shape[0]-(img1.shape[0]*2*factor_y) - 755, 'spheroid edge', color="red", fontsize = 14)
    ax3.text(img1.shape[1]-(img1.shape[1]*2*factor_x) - 370, img1.shape[0]-(img1.shape[0]*2*factor_y) - 680, 'd/2 distance', color="#C3E994", fontsize = 14)
    ax3.axis('off')
    #plt.show()

    # writing info box in plot
    meta_info_sel=dict((k, meta_info[k]) for k in ("rep","pos","well","Mic","file_path")) # possible selection to be written in info box
    ...
    infostr2 = "\n".join(['%s: %s' % (key, value) for (key, value) in list(meta_info_sel.items())[list_length//2:list_length]])
